...
export function Dashboard({ currentUser }) {
  ...
  const { data: activity }       = useData('activity',       FALLBACK_ACTIVITY);
  ...
  const recurring  = qualityIssues.filter(i => i.recurring && i.status === 'open').length;

  const stats = [
    { label:'Dự Án Đang Chạy', value:projects.filter(p=>p.status==='active').length, icon:'◈', color:'#60a5fa' },
    { label:'Task Của Tôi',    value:myTasks.length,                                  icon:'◎', color:'#4ade80' },
    { label:'Lỗi Đang Mở',    value:openIssues,                                      icon:'△', color:'#f59e0b' },
    { label:'Lỗi Tái Phát',   value:recurring,                                       icon:'⚠', color:'#ef4444' },
  ];
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        ...
        <Card>
          ...
          {activity.map(a => {
            ...
            if (!u) return null;
            return (
              <div key={a.id} style={{ display:'flex', gap:10, alignItems:'flex-start', marginBottom:12 }}>
      ...
      <Card>
        ...
        {myTasks.length ? myTasks.map(t => (
          <div key={t.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px', background:'#ffffff04', borderRadius:8, border:'1px solid #ffffff08', marginBottom:6 }}>
            <div style={{ width:8, height:8, borderRadius:'50%', background:PRIORITY_COLORS[t.priority], flexShrink:0 }} />
            <span style={{ flex:1, fontSize:13 }}>{t.title}</span>
            <Badge color={STATUS_COLORS[t.status]}>{t.status==='inprogress'?'Đang làm':t.status==='review'?'Duyệt':'Chờ'}</Badge>
